...
FILES
   ...
   Receiver.mon                          An EPL receiver to receive email
   Sender.mon                            An EPL sender to send email
...
RUNNING THE APPLICTION
   ...
   4. Inject the Receiver.mon into the receiver correlator to receive email

      > engine_inject Receiver.mon

   5. Inject the Sender.mon  into the sender correlator to send email

      > engine_inject -u Sender.mon --port 15904
Output
  Check the receiver correlator log for the following
  
   Got kitten price: KittenPrice("Fluffy Kitten £240.30")
